...
AudialVisualization::AudialVisualization(int argc, char** argv) {
  ...
  if (result["canvas-width"].as<int>() > 0) {
    if (result["canvas-width"].as<int>() <
        100) {  // don't make the canvas any smaller than this
      setCanvasWidth(100);
    }
...
void AudialVisualization::setNumThreads(const int n) { num_threads = n; }

void AudialVisualization::setCanvasHeight(const int h) { canvas_height = h; }

void AudialVisualization::setCanvasWidth(const int w) { canvas_width = w; }
...
int AudialVisualization::getNumThreads() const { return num_threads; }

int AudialVisualization::getCanvasHeight() const { return canvas_height; }

int AudialVisualization::getCanvasWidth() const { return canvas_width; }
